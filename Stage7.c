...
int main(){
	...
	//turns are taken cyclicaly based on remainder values
        ...
        //3 player mode
            ...
            if(activeCount == 1){
                if(p1.active) printf("Player @ wins!\n");
                if(p2.active) printf("Player & wins!\n");
                if(p3.active) printf("Player $ wins!\n");
                break;
            }
...
void playerMovement(Player *p, char **grid, int N, char move){
    ...
    if(grid[newRow][newCol] == 'X'){
        if(p->intel == 3){
            ...
            printf("Player %c reached X without collecting all intel. Eliminated!\n",p->symbol);
            p->active = 0;
            return;
        }
    }
...
void logGame(Player *p, char **grid, int N){
    ...
    fprintf(fp,"Player lives: %d | Intel: %d\n",p->lives,p->intel);
